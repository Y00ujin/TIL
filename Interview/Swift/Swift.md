...
- ### 프로퍼티의 종류에 대해 설명하시오.
##### 먼저 저장프로퍼티는 클래스와 구조체에서만 사용할 수 있고, 값을 저장하기 위해 선언되는 상수 또는 변수이며 지연 저장 프로퍼티는 프로퍼티가 호출되기 전까지는 초기화되지 않고 있다가, 프로퍼티가 호출되는 순간에 초기화되는 저장 프로퍼티입니다. 연산 프로퍼티는 클래스, 구조체, 열거형에서 사용되며, 저장프로퍼티와 달리 저장 공간을 갖지 않고, 다른 저장 프로퍼티의 값을 읽어 연산을 실행하거나, 프로퍼티로 전달받은 값을 다른 프로퍼티에 저장하기 때문에 항상 var로 선언되어야 합니다. get-only는 가능하지만 set-only는 불가능합니다. 타입 프로퍼티는 저장 타입 프로퍼티와 연산 타입 프로퍼티가 있으며, 저장 타입 프로퍼티는 초깃값이 있어야하며, 변수와 상수 모두 선언가능합니다. 연산 타입 프로퍼티는 값을 저장하는 게 아니므로 var로만 정의가 가능하고, class로 선언시에는 오버라이딩이 가능하며, static으로 선언할 시에는 오버라이딩이 불가능합니다.
